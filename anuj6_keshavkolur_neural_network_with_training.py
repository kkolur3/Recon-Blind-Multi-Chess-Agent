...
def run_input(input_tensor, action_tensor, hidden_state_tensor, cell_state_tensor):
                ...
                if "game" in filename and "Game Over" in gameOverLine:
                    ...
                    for line in lineList:
                        ...
                        if turn and "Move" in line:
                            ...
                            probs, hidden = run_input(prevBoardDist.reshape((1, 64, 7, 1)),
...
def train_network(iterations):
    saver = tf.train.Saver()
    # saver.restore(sess, "cur_model.ckpt")
    with (open("episodes.pkl", "rb")) as f:
        episodes = pickle.load(f)
    print(type(episodes))
    for episode in episodes:
        ...
        for i in range(iterations):
            ...
            probs, hidden = run_input(np.array(final_obs).reshape((1, 64, 7, 1)),
...
def make_move(state, possible_moves):
    ...
    for move in possible_moves:
        ...
        probs, hidden = run_input(boardDist, action, observedBoard, beliefBoard)
