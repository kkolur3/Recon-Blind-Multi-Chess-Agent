...
# Create CNNs
c1_weights = tf.get_variable("c1_weights", shape=(3, 3, 1, 4),
                                 dtype="float32", initializer=tf.contrib.layers.xavier_initializer())
c1_bias = tf.get_variable("c1_bias", shape=4, initializer=tf.constant_initializer(0.0))
c2_weights = tf.get_variable("c2_weights", shape=(3, 3, 4, 8),
                                 dtype="float32", initializer=tf.contrib.layers.xavier_initializer())
c2_bias = tf.get_variable("c2_bias", shape=8, initializer=tf.constant_initializer(0.0))
c3_weights = tf.get_variable("c3_weights", shape=(3, 3, 8, 16),
                                 dtype="float32", initializer=tf.contrib.layers.xavier_initializer())
c3_bias = tf.get_variable("c3_bias", shape=16, initializer=tf.constant_initializer(0.0))

# Run board through CNNs
c1 = tf.nn.relu(tf.nn.conv2d(board, c1_weights, strides=[1, 1, 1, 1], padding="SAME") + c1_bias)
pool1 = tf.nn.max_pool(c1, strides=[1, 2, 2, 1], ksize=[1, 2, 2, 1], padding="SAME")
bn1 = tf.layers.batch_normalization(pool1)

c2 = tf.nn.relu(tf.nn.conv2d(bn1, c2_weights, strides=[1, 1, 1, 1], padding="SAME") + c2_bias)
pool2 = tf.nn.max_pool(c2, strides=[1, 2, 2, 1], ksize=[1, 2, 2, 1], padding="SAME")
bn2 = tf.layers.batch_normalization(pool2)

c3 = tf.nn.relu(tf.nn.conv2d(bn2, c3_weights, strides=[1, 1, 1, 1], padding="SAME") + c3_bias)
pool3 = tf.nn.max_pool(c3, strides=[1, 2, 2, 1], ksize=[1, 2, 2, 1], padding="SAME")
bn3 = tf.layers.batch_normalization(pool3)

# Run action through fully connected layer
action_weights = tf.get_variable(name="action_weights", shape=[num_outputs, rnn_size], dtype="float32",
                             initializer=tf.contrib.layers.xavier_initializer())
action_bias = tf.get_variable(name="action_bias", shape=rnn_size, dtype='float32',
                                  initializer=tf.constant_initializer(0.0))
action_layer = tf.nn.tanh(tf.add(tf.matmul(actions, action_weights), action_bias))

# Combined board ran through CNN with action taken
batch_reshaped = tf.reshape(bn3, shape=(-1, rnn_size))
board_action = tf.concat([batch_reshaped, action_layer], axis=-1)
...
board_action_output = tf.add(tf.matmul(board_action, combined_action_board_weights), combined_action_board_bias)

# Run through RNN
flattened = tf.contrib.slim.flatten(board_action_output)
...
def input_pass(input_tensor, action_tensor, hidden_state_tensor, cell_state_tensor):
...
def init_dist(color):
    ...
    for i in range(64):
        if i == 0 or i == 7 or i == 56 or i == 63: # ROOK
            square_dist = [0, 0, 0, 0, 1, 0, 0]
        elif i == 1 or i == 6 or i == 57 or i == 62: # KNIGHT
            square_dist = [0, 0, 1, 0, 0, 0, 0]
        elif i == 2 or i == 5 or i == 58 or i == 61: # BISHOP
            square_dist = [0, 0, 0, 1, 0, 0, 0]
        elif i == 3 or i == 59: # QUEEN
            square_dist = [0, 0, 0, 0, 0, 1, 0]
        elif i == 4 or i == 60: # KING
            square_dist = [0, 0, 0, 0, 0, 0, 1]
        elif (i > 7 and i < 16) or (i > 47 and i < 56):
            square_dist = [0, 1, 0, 0, 0, 0, 0]
        else:
            square_dist = [0, 0, 0, 0, 0, 0, 0]
        if (i < 16 and color == chess.WHITE) or (i > 47 and color == chess.BLACK):
            square_dist[0] = 1
        dist[i] = square_dist
    return dist
...
def create_episodes():
    ...
    for color in [chess.WHITE, chess.BLACK]:
        ...
        action_tensor = np.zeros(shape=(1, 64*82))
                ...
                if "game" in filename and "Game Over" in gameOverLine:
                    ...
                    for line in lineList:
                        ...
                        if turn and "Move" in line:
                            ...
                            probs, hidden = input_pass(prevBoardDist.reshape((1, 64, 7, 1)),
...
def train_network(iterations):
    ...
    for episode in episodes:
        ...
        for i in range(iterations):
            ...
            probs, hidden = input_pass(np.array(final_obs).reshape((1, 64, 7, 1)),
            ...
            curLoss, _ = sess.run([loss, train], feed_dict={board: np.array(curBoards).reshape((num_turns, 64, 7, 1)),
...
def make_move(state, possible_moves):
    ...
    for move in possible_moves:
        ...
        probs, hidden = input_pass(boardDist, action, observedBoard, beliefBoard)
